    ...
    <section class="section">
        <div class="row">
            <div class="col-lg-6">
                <div class="card">
                    <div class="card-body">
                        <form class="row g-3 mt-3" enctype="multipart/form-data" name="edit_user_form" method="post" action="{{ route('admin.users.update') }}">
                            @csrf
                            ...
                            <div class="col-12">
                                ...
                                <select class="form-control" name="type" id="type">
                                    ...
                                    <option value="{{ config('constants.USER_TYPE.TEACHER') }}" {{ $user->type == config('constants.USER_TYPE.TEACHER') ? 'selected':'' }}>Tutor</option>
                                    <option value="{{ config('constants.USER_TYPE.STUDENT') }}" {{ $user->type == config('constants.USER_TYPE.STUDENT') ? 'selected':'' }}>Student</option>
                                </select>
                            </div>
                            @if($user->type==3)
                                <div class="col-12" id="cc_emails_container">
                                    <label for="cc_emails" class="form-label">CC Emails (Add ; only between email)</label>
                                    <input type="text" class="form-control" value="{{ $user->cc_mails }}" name="cc_emails" id="cc_emails">
                                </div>
                                @endif
                            <div class="col-12">
     ...
     last_name: {
      ...
      email: {
        ...
        remote: {
            ...
            url:"{{ route('admin.users.email.exist') }}",
            type:'POST',
